...
const TopPlay = () => {
  ...
  const { data } = useGetTopChartsQuery();  //can't change the name of the data
        ...
        >
          {topPlays?.map((song, i) => (
            <SwiperSlide
              ...
              className="shadow-lg rounded-lg animate-sliderright"
            >
              <Link to={`/artists/${song?.artists[0]?.adamid}`}>
                <img alt={song?.title} src={song?.images?.background} className="rounded-full w-full object-cover" />
              </Link>
            </SwiperSlide>
          ))}
        </Swiper>
